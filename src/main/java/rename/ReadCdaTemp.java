...
public class ReadCdaTemp
{
    public static void main(String[] args) throws IOException
    {
        ...
        for (int i = 1; i <= shujujiList.length; i++)
        {
            ...
            System.out.println("****************"+cdaFileNames[i - 1]+"*****************");
            HashSet<String> strings = new HashSet<>();
            for (String s : metadataSet)
            {

                String[] split = s.split("\\.");
                strings.add(split[0].replace("{",""));
            }
...
/*
    ...
    private static HashSet<String> getMetadata(String reallFileAddress) throws IOException
    {
        ...
        while (m.find())
        {
            ...
            if (group.indexOf("dict") == -1 && group.indexOf("docInfo") == -1 && group.indexOf("privacy") == -1)
            {
                res.add(group);
            }
